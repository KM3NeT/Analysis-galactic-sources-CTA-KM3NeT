...
for source in tqdm(source_files, "FIT"):
    ...
    path_FP = Path(parpar_dir / "data" / "models" / f"{source[0]}.csv")
    ...
    ECPL_PD = ExponentialCutoffPowerLaw(
        amplitude=source[4][0] / u.eV,
        e_0=source[4][1] * u.TeV,
        alpha=source[4][2],
        e_cutoff=source[4][3] * u.TeV,
        beta=source[4][4],
    )
    ...
    ECPL_IC = ExponentialCutoffPowerLaw(
        amplitude=source[5][0] / u.eV,
        e_0=source[5][1] * u.TeV,
        alpha=source[5][2],
        e_cutoff=source[5][3] * u.TeV,
        beta=source[5][4],
    )
    ...
    popt_ECPL_PD, cov_ECPL_PD = curve_fit(
        ...
        p0=source[6],  # initial guess for parameters
    )
    ...
    popt_ECPL_IC, cov_ECPL_IC = curve_fit(
        ...
        FP_df["dnde"],  # flux point values
        sigma=FP_df["dnde_err"],  # flux point errors
        p0=source[7],  # initial guess for parameters
    )
    ...
    # save output of fit
    np.savetxt(
        Path(parpar_dir / "data" / "models" / f"input_model_PD_{source[1]}.txt"),
        gamma_ECPL_PD.parameters.values,
    )

    np.savetxt(
        Path(parpar_dir / "data" / "models" / f"input_model_IC_{source[1]}.txt"),
    ...
    with warnings.catch_warnings():
        ...
        FP.plot(energy_power=2, marker=".", markersize=5, label=source[3], ax=ax)
        gamma_ECPL_IC.plot(
            source[8] * u.TeV,
            energy_power=2,
    ...
    ax.text(0.97, 0.95, source[2], ha="right", va="top", transform=ax.transAxes)
    ax.legend(loc="lower left")

    plot_utils.format_log_axis(ax.xaxis)

    for form in ["png", "pdf"]:
        fig.savefig(Path(output_path / f"fit_{source[0]}.{form}"), dpi=300)
